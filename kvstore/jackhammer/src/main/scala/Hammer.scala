...
trait ClusterSettings {
  ...
  val grepPath = "src/main/bash/grep.sh"
}
...
trait SourceSettings {
  ...
  val configScalaPath:Option[String]
}
...
object Util {
  ...
  def execute(cmd:String): Tuple2[String,String] = execute(cmd:String,"")
  ...
  def getDesigns(grepPath:String,config:String,configScalaPath:String):List[String] = {
    val out = execute("./" + grepPath + " " + config + " " + configScalaPath)
    out._1.split("\n").toList
  }
}
  ...
  def main(args: Array[String]): Unit = {
    ...
    if(mode.get == "c") {
      ...
      if(settings.no_nscratch){
        execute("bash "+settings.copyScript,design+settings.version)
      }
    }
    if(mode.get == "p") {
      val designs = if(!settings.config.isEmpty && !settings.configScalaPath.isEmpty) {
        execute("make",settings.config.get);
        getDesigns(settings.grepPath,settings.config.get,settings.configScalaPath.get)
      } else settings.designs

      execute(Seq(settings.setupPath, settings.baseShared, settings.root).mkString(" "))
      if(!designs.isEmpty && designs.filter(_ == "").isEmpty) {
        designs.foreach( d => {
      ...
      }else println("No designs to Run!")
    }
    if (mode.get == "u") {
    }
  }
